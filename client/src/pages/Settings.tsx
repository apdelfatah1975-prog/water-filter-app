...
import { ArrowLeft, Eye, EyeOff, Image as ImageIcon, KeyRound, RotateCcw, Save, ShieldCheck, SlidersHorizontal, Smile, Trash2, Undo2 } from "lucide-react";
...
export default function Settings() {
  ...
  const whatsappSectionRef = useRef<HTMLDetailsElement | null>(null);
  ...
  useEffect(() => {
    ...
    if (section === "trash") {
      ...
      scrollTimer = window.setTimeout(() => whatsappSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
    } else if (section === "sync") {
      scrollTimer = window.setTimeout(() => syncSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 80);
    }
  ...
  function openWhatsAppPanel(panel: "manual" | "official") {
    setActiveWhatsAppPanel(panel);
  ...
  return <div className="mx-auto max-w-5xl space-y-6" dir="rtl">
    ...
    <details ref={whatsappSectionRef} className="soft-card overflow-hidden group" open><summary className="cursor-pointer list-none border-b border-teal-950/6 bg-teal-50/40 px-5 py-3 text-sm font-extrabold text-teal-900 marker:hidden">فتح أو إخفاء هذا القسم</summary><div className="border-b border-teal-950/6 p-5"><div className="flex items-start gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-emerald-100 text-emerald-700"><span className="text-lg font-black">WA</span></div><div><h2 className="font-extrabold">استيراد مواقع واتساب</h2><p className="mt-1 text-xs leading-6 text-muted-foreground">الخيار اليدوي يعمل الآن من نموذج العميل: الصق رابط الموقع أو الإحداثيات من واتساب وسيحفظ التطبيق الرابط ويستخرج الإحداثيات المتاحة. أما الاستيراد التلقائي فيحتاج حساب WhatsApp Business Platform رسميًا.</p></div></div></div><div className="grid gap-3 p-5 sm:grid-cols-2"><button type="button" data-testid="whatsapp-manual-card" aria-pressed={activeWhatsAppPanel === "manual"} onClick={() => openWhatsAppPanel("manual")} className={`group rounded-2xl border p-4 text-right transition active:scale-[.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 ${activeWhatsAppPanel === "manual" ? "border-emerald-500 bg-emerald-100 shadow-sm" : "border-emerald-200 bg-emerald-50 hover:border-emerald-400 hover:bg-emerald-100"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-extrabold text-emerald-900">الخيار اليدوي</p><p className="mt-1 text-xs leading-6 text-emerald-800">متاح دون إعدادات إضافية، ومناسب عندما يرسل العميل الموقع في محادثة واتساب عادية.</p></div><ArrowLeft className="mt-1 h-4 w-4 shrink-0 text-emerald-700 transition-transform group-hover:-translate-x-1" aria-hidden="true" /></div><span className="mt-3 inline-flex rounded-full bg-white px-3 py-1 text-xs font-extrabold text-emerald-700">مفعّل الآن — اضغط لفتح طريقة الاستخدام</span></button><button type="button" data-testid="whatsapp-official-card" aria-pressed={activeWhatsAppPanel === "official"} onClick={() => openWhatsAppPanel("official")} className={`group rounded-2xl border p-4 text-right transition active:scale-[.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-600 ${activeWhatsAppPanel === "official" ? "border-sky-500 bg-sky-100 shadow-sm" : "border-sky-200 bg-sky-50 hover:border-sky-400 hover:bg-sky-100"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-extrabold text-sky-950">التكامل الرسمي API</p><p className="mt-1 text-xs leading-6 text-sky-900">يتطلب Meta Business، رقم WhatsApp Business، وPhone Number ID وAccess Token وWebhook. لا نضع رمز الوصول داخل الواجهة أو في المتصفح.</p></div><ArrowLeft className="mt-1 h-4 w-4 shrink-0 text-sky-700 transition-transform group-hover:-translate-x-1" aria-hidden="true" /></div><span className="mt-3 inline-flex rounded-full bg-white px-3 py-1 text-xs font-extrabold text-sky-700">غير مهيأ حالياً — اضغط لعرض المتطلبات</span></button></div>{false && activeWhatsAppPanel ? <div data-testid="whatsapp-card-panel" className="mx-5 mb-5 rounded-2xl border border-teal-200 bg-white p-4 shadow-sm">{activeWhatsAppPanel === "manual" ? <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-extrabold text-emerald-900">طريقة استيراد موقع من واتساب</h3><p className="mt-1 text-sm leading-6 text-slate-600">افتح نموذج عميل جديد، ثم الصق رابط الموقع أو الإحداثيات التي أرسلها العميل في حقل الموقع. سيحتفظ التطبيق بالرابط ويستخرج الإحداثيات المتاحة تلقائياً.</p></div><Button type="button" onClick={() => setLocation("/customers/new")} className="shrink-0 rounded-xl bg-emerald-700 hover:bg-emerald-800"><ArrowLeft className="ml-1 h-4 w-4" />فتح نموذج العميل</Button></div> : <div><h3 className="font-extrabold text-sky-950">التكامل الرسمي Meta WhatsApp Cloud API</h3><p className="mt-1 text-sm leading-6 text-slate-600">الحالة الحالية: <strong>غير مهيأ</strong>. هذه الواجهة جاهزة مستقبلاً لإضافة بيانات Meta من خلال إعداد آمن على الخادم، ولا تطلب أي أسرار الآن ولا ترسل رسائل فعلية.</p><div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-sky-900"><span className="rounded-full bg-sky-50 px-3 py-1">Meta Business</span><span className="rounded-full bg-sky-50 px-3 py-1">Phone Number ID</span><span className="rounded-full bg-sky-50 px-3 py-1">Access Token</span><span className="rounded-full bg-sky-50 px-3 py-1">Webhook</span></div></div>}</div> : null}</details>
    ...
    <details ref={syncSectionRef} className="soft-card overflow-hidden group" open><summary className="cursor-pointer list-none border-b border-teal-950/6 bg-teal-50/40 px-5 py-3 text-sm font-extrabold text-teal-900 marker:hidden">فتح أو إخفاء هذا القسم</summary><div className="border-b border-teal-950/6 p-5"><h2 className="font-extrabold">العمل دون إنترنت والمزامنة</h2><p className="mt-1 text-xs leading-6 text-muted-foreground">هذه الخيارات تحافظ على سرعة التسجيل وتحدد سلوك المزامنة عند عودة الإنترنت.</p></div><div className="grid gap-3 p-5 sm:grid-cols-2"><Toggle checked={settings.autoSaveLocally} onChange={value => update("autoSaveLocally", value)} label="الحفظ المحلي التلقائي لكل عملية" /><Toggle checked={settings.syncWhenOnline} onChange={value => update("syncWhenOnline", value)} label="المزامنة تلقائيًا عند عودة الإنترنت" /><Toggle checked={settings.confirmDestructiveActions} onChange={value => update("confirmDestructiveActions", value)} label="طلب تأكيد قبل الحذف أو الاستعادة" /></div><div className="mx-5 mb-5 flex flex-col gap-3 rounded-2xl border border-sky-200 bg-sky-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-extrabold text-sky-950">حالة الاتصال والمزامنة</p><p className="mt-1 text-xs leading-6 text-sky-900">افتح شاشة الحالة لرؤية اتصال الخادم وتفاصيل التحديث المركزي.</p></div><Button type="button" onClick={() => setLocation("/pending-operations")} className="shrink-0 rounded-xl bg-sky-700 hover:bg-sky-800"><ArrowLeft className="ml-1 h-4 w-4" />فتح حالة المزامنة</Button></div></details>
    ...
    <Dialog open={whatsappDialogOpen} onOpenChange={open => open ? setWhatsAppDialogOpen(true) : closeWhatsAppPanel()}>
      <DialogContent dir="rtl" data-testid="whatsapp-card-dialog" className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{activeWhatsAppPanel === "manual" ? "إعداد واتساب اليدوي" : "إعداد التكامل الرسمي"}</DialogTitle>
          <DialogDescription>تُفتح هذه الشاشة مباشرة من البطاقة التي اخترتها، ويمكن إغلاقها من زر الإغلاق أو بمفتاح Escape.</DialogDescription>
        </DialogHeader>
        {activeWhatsAppPanel === "manual" ? <div data-testid="whatsapp-card-panel" className="space-y-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-950">
